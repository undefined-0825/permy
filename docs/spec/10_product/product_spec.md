...
**Last Updated (JST):** 2026-03-03 09:30:00 +0900
...
- [ ] 設定画面から遷移できる「利用規約」ページを実装（または外部URL導線を実装）。
- [ ] 設定画面から遷移できる「プライバシーポリシー」ページを実装（または外部URL導線を実装）。
- [ ] 設定画面から遷移できる「ヘルプ（使い方）」ページを実装。
- [ ] Google Play / App Store の公開情報に、上記法務ページURLを登録。
- [ ] 課金導線を出す場合は「購入復元」導線を実装し、ストア審査要件を満たす。
- [ ] 課金導線を出す場合は「サブスクリプション管理（解約）」導線を実装。
- [ ] アカウント削除/データ削除要件に備え、削除導線（UI）とAPI方針をSpecで確定。
- [ ] OSSライセンス表示導線を設定画面に実装（Flutter標準のライセンス画面可）。
