...
抽象論ではなく、Permyの制約に合わせて **具体ルール**として固定する。
...
- 実装はCopilot中心。Specの更新はPR提案のみ（ちょび承認必須）。
- **本文/生成文を保存・ログ出力しない**（端末/サーバ/CI/監視すべて）。

---

## 1. プロンプト構造化ルール（MUST）

Copilotへ依頼するときの指示は以下の内容を理解して書く（状況により省略・順序入替を許可）

1) **Role**: `Flutter Engineer` または `FastAPI Engineer`
2) **Read First**: 必ず読むファイルを列挙（パス固定）
3) **Goal**: 何を作るかを一文で
4) **Files to Edit/Create**: 変更対象のファイルパスを列挙（新規作成含む）
5) **Constraints**: 本プロジェクト固有のMUSTを列挙
...
- **Flutter UI**: 表示・入力・画面遷移のみ（ビジネス判断を持たない）
- **Flutter Domain**: 判定/変換/ルール（純粋関数寄り、I/Oなし）
- **Flutter Infra**: HTTP/Share受信/端末機能（I/O）
- **FastAPI Router**: ルーティングとDTOバリデーション
- **FastAPI Service**: ユースケース（NG適用、生成制御、メタ生成）
- **FastAPI Infra**: OpenAIクライアント、永続化、テレメトリ送信先
...
## 4. テストファースト（実装と同時にテスト生成 / MUST）

### 4.1 絶対ルール（MUST）

- 実装コード生成時、**同時にテストコードも生成**する。
- 修正時も、必ずテストを更新する（テスト無し修正禁止）。

### 4.2 FastAPI（MUST）

- `backend/tests/` に以下を作る：
  - `test_contract_*.py`（API契約テスト）
  - `test_logic_*.py`（NG優先順位、followup等のロジック）
- Contractは request/response の必須キーと型を検証する（本文は扱わない）。
- Unitは OpenAIをモック。
...
以下は **保存・ログ出力・テレメトリ送信を一切禁止**：
- LINEトーク履歴txtの本文
- 生成した返信案の本文
- NGフレーズの生テキスト（必要なら件数/ハッシュのみ）

エラー時も本文を出さない。返すのは **error_code / request_id / 要約**のみ。
...
- レスポンスは「本文なし」設計で、必要なメタ（plan/daily/request_id等）を返す。
- ログは「本文なし」。error_code と request_id 中心。
...
- A/B/Cカードは省略表示禁止（maxLines固定禁止）。コピーは全文。
- 共有受信（Android/iOS）はラッパーで隠蔽し、UIは「txt受領」だけにする。

---

## 8. 禁止事項（NG）

- Specにない機能追加（登録画面、貼り付け欄、本文保存、勝手なSDK導入）
- 既決定事項の変更（価格/回数/導線/保存禁止/禁止語）
- 大規模リファクタ（必要箇所以外を触る）
