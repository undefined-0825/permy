...
- `real_self_type`（必須）
- `night_self_type`（必須）
- `persona_version=3`（必須）
- `true_self_type` は扱わない（受理しない/保存しない）。

## AB-2. PersonaSetup 実装

- 6問（Night3 + Real3）を順次表示し、加点マップ（+2/+1）で集計する。
- 同点の場合は追加の決選1問を表示し、同点タイプから1つ確定する。

## AB-3. /generate 送信時

- `settings_snapshot` に `real_self_type` と `night_self_type` を必ず含める。
- `client_now`（端末時刻ISO8601, JST）を送る（時間整合性のため）。

---

# 付録AC: 長文寄り返信のUI実装（MUST）

**Added (JST):** 2026-03-01 23:35:00 +0900

- A/B/Cカードの本文は `maxLines` を固定しない（省略表示禁止）。
- カードはスクロール可能領域内に置き、長文でも操作が破綻しないこと。
- コピーは表示全文を対象とする（省略テキストをコピーしない）。
