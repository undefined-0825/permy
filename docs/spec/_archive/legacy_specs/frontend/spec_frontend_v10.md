...
# 付録AB: ペルソナ診断UI（確定）反映（MUST）
**Added (JST):** 2026-03-01 23:05:00 +0900

- PersonaSetupは以下の順に表示する:
...
- 同点時は「追加設問（決選）」画面を表示する（内容は今後確定）。暫定として:
  - 同点タイプが2〜3の時だけ表示
  - 選択肢は同点タイプ名（日本語ラベル）をそのまま提示
- 結果画面の表示名は product Spec の AB-2 の日本語表記をSSOTとする。
- 結果保存:
  - `real_self_type`
  - `night_self_type`
  - `persona_version=3`
...
- 返信案（A/B/C）は短文縛りをしないため、カード内は **複数行表示**を前提にする。
- 行数が増えても読めるよう、カード内の行間/余白は確保する（Safe Area規約は既存通り）。
