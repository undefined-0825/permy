...
# 付録AA: 環境構築/疎通の既知落とし穴（MUST / 手戻り防止）
**Added (JST):** 2026-03-01 20:05:00 +0900

## AA-1. DB初期化の必須条件（再掲・強化）
- `python -m app.scripts.init_db` は **必ず backend 直下（DB相対パスが解決できる場所）で実行**する。
- `app/scripts/init_db.py` は `import app.models` を必須とする（`Base.metadata` が空のまま `create_all` して空DBになる事故を防ぐ）。fileciteturn29file14L22-L25

## AA-2. .env の読み込み（MUST）
- `app.config.settings` は **`.env` をSSOTとして読み込む**（Pydantic Settings の `env_file=.env` 等）。
- `.env` 変更は **必ずサーバ再起動で反映**する（ホットリロード前提にしない）。

## AA-3. REDIS_DISABLED の反映（MUST）
- `REDIS_DISABLED=true` は `.env` と環境変数の双方で許可する。
- ただし、環境差の事故を避けるため、実装は `settings.redis_disabled`（bool）に集約し、`redis_client` はそれを参照する。
- 開発でメモリRedisを使う場合、**サーバ再起動でセッションが失効**するため、疎通テストは「/auth/anonymous→/generate」を同一セッションで実行すること。

## AA-4. PowerShell 5系でのHTTP疎通（MUST）
- Windows PowerShell 5系では `Invoke-WebRequest -SkipHttpErrorCheck` が使えないため、エラー本文取得は `try/catch` + `GetResponseStream()` 方式で行う。

## AA-5. OpenAI SDKの固定（MUST）
- OpenAI SDKは `openai==1.99.0` に固定する（混在/破損による ImportError を防ぐ）。
- `pip freeze | findstr /i openai` で **openaiが1行**であることを確認する。

## AA-6. OPENAI_API_KEY の参照優先順位（MUST）
- `.env` と OS環境変数の両方に `OPENAI_API_KEY` が存在する場合、**環境変数が優先**され得る。
- 疑義がある場合は、起動前に `OPENAI_API_KEY` の環境変数を削除してから起動する（PowerShell例）:
  - `Remove-Item Env:OPENAI_API_KEY -ErrorAction SilentlyContinue`
- サーバが参照しているキーは、ログに出さず **prefix/len/tail(末尾4)のみ**で確認する（鍵漏洩防止）。
