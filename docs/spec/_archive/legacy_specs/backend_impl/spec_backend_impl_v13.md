...
# 付録AA: セッションTTLの最終確定（MUST）
**Added (JST):** 2026-03-01 21:12:00 +0900

- Opaqueセッション（Redis `sess:{token}`）のTTLは **30日固定**。
- 14日などの暫定値は **採用しない**。本書内に旧記載が残っていても、本付録で上書きする。

---

# 付録AB: 実運用で発生した事故の再発防止（MUST）
**Added (JST):** 2026-03-01 22:00:00 +0900

## AB-1. Windows PowerShell制約（bash流用禁止）
- Windows PowerShell では bash のヒアドキュメント（例：`python - << 'PY'`）は使用できない。
- 複数行Pythonを実行する場合は、以下のいずれかを用いる（推奨順）:
  1) 一時 `.py` を `@' ... '@ | Set-Content` で作成して実行
  2) `python -c "..."`（短い1行のみ。クォート崩れが起きやすいので推奨しない）

## AB-2. requirements.txt の文字化け/コメント化事故（MUST）
- `requirements.txt` は **UTF-8** で保存する（文字化けを許容しない）。
- 依存が `#` コメント化されていないことを確認する（openai/SQLAlchemy/redis/tzdata等が欠落すると起動できない）。
- 依存が壊れている疑いがある場合、Stepを進める前に **必ず** 次を実行して事実確認する:
  - `Get-Content .\requirements.txt`
  - `python -m pip install -r .\requirements.txt`

## AB-3. 起動前の import 一括検査（MUST）
- 受け入れ条件 Step 4（起動）へ進む前に、依存の欠落を機械的に潰すため、以下の import 一括検査を実行する:
  - `python -c "import fastapi,uvicorn,pydantic,pydantic_settings,sqlalchemy,aiosqlite,redis,tzdata,openai; print('imports OK')"`

## AB-4. `from __future__ import annotations` の配置規約（MUST）
- `from __future__ import annotations` を使用する場合、**ファイルの先頭（コメント/空行以外の最初）**に置く。
- 先頭に他のimport等がある状態でコミットしない（起動時に `SyntaxError` で落ちるため）。

## AB-5. Copilot指示ファイルの文字化け防止（SHOULD）
- `.github/copilot-instructions.md` は **UTF-8** で保存する（文字化け＝指示破壊を招く）。
- 破壊操作・手順固定・PowerShell前提・SSOT入口の指示は **短く一意**に保つ（重複/矛盾は禁止）。

---

# 付録AC: 生成ロジックの確定反映（MUST）
**Added (JST):** 2026-03-01 23:05:00 +0900

## AC-1. json_schema（Structured Outputs）更新
- A/B/C それぞれの `text` は **140文字以内**（Unicode code point）をMUSTとする。fileciteturn38file0L106-L107
- 句読点（。、）の多用を避ける（プロンプトで強制）。
- 生成は **1吹き出し**（長文分割は禁止）。改行は許容（リズム用途）。

## AC-2. プロンプト組み立て（MUST）
- `NightSelfType` と `RealSelfType` を明示して、タイプ別NGルールを付与する。fileciteturn38file0L7-L15 fileciteturn38file0L109-L115
- 履歴ミラーリング（語彙/改行/絵文字量）を最優先する。fileciteturn38file0L102-L107
- 呼称推定:
  - 履歴から相手への呼称（例: 〇〇くん/〇〇さん/ニックネーム）を推定する。
  - **第三者名の誤認は致命的NG**。判定不能なら呼称を入れない。fileciteturn38file0L11-L15

## AC-3. 時間整合性（MUST）
- `client_now` と、履歴に含まれる最終時刻（抽出できた場合）から、挨拶を自然にする。fileciteturn38file0L105-L107
- 時刻抽出不能時は、挨拶を省略してよい（誤爆回避）。

## AC-4. followup（同点決選の暫定運用）
- 診断同点時は、PersonaSetupで追加設問（決選）を表示する。
- 決選設問が未定義の間は、同点時に限り `followup.key="persona_tiebreak"` として1問を返してよい。
  - choicesは同点タイプ名に対応（最大3）とする。

---

# 付録AD: 返信長さポリシーの適用（MUST）
**Added (JST):** 2026-03-01 23:35:00 +0900

- Structured Outputs の json_schema で「140文字以内」等のハード制約を設定しない（採用しない）。
- プロンプト規約:
  - 履歴ミラーリングに従い、**長文寄り**の返信を生成する（短文縛り禁止）。
  - 改行で読みやすさを作る（2〜6行目安）。
  - 丁寧すぎる敬語/過剰な句読点/高い漢字率は排除（人間味フィルタは維持）。
