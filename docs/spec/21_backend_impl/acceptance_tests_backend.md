...
> 本文（LINE履歴txt・生成文）を保存しない制約が最優先。テストでも本文を永続化しない。
...
- `plan` は free/pro のいずれか
- `settings_json` に本文/生成文が含まれない
...
- `ng_free_phrases` が1要素あたり長すぎる場合は 400/422（推奨：上限 64文字程度）
...
- `followup` は必要時のみ（不足時）
- サーバは txt本文/生成文を永続化しない

> 注：OPENAI_DISABLED=true の時、返信案本文は「固定テンプレ」でもよいが、形式は本番と同じにする（フロント実装を進めるため）。
...
- A/B/Cは返す（仮案）
- `followup` が返る：
  - `key` は許可リスト内（relationship_type/reply_length_pref/ng_tags/ng_free_phrases）
  - `question` があり、`choices` は 1..3
- サーバは followup を本文扱いで保存しない
...
$body = @{
  ...
  ng_tags = @("no_pressure","no_preach")
  ng_free_phrases = @("会いたい")
} | ConvertTo-Json -Depth 5

curl -s -X PUT -H "Authorization: Bearer $token" -H "Content-Type: application/json" -d $body "$base/api/v1/me/settings"
```
...
curl -s -X POST -H "Authorization: Bearer $token" -H "Content-Type: application/json" -d $g "$base/api/v1/generate"
```
