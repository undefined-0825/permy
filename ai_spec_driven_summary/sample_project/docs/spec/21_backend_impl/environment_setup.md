# environment_setup.md — [ProjectName] Backend 環境セットアップ（開発 / CI / Render）

**Scope:** バックエンド（FastAPI）の開発環境・CI・Renderテスト運用のセットアップ手順を定義する。  
●●非保存ポリシーにより、ログ/テストデータの取り扱いに注意する。

---

## 0. 前提
- OS: Windows 10/11（PowerShell 5系での運用を想定する場合は別途●●ジェクト標準に従う）
- Python: 3.12+（●●ジェクト標準に合わせる）
- Git: 最新安定版
- エディタ: VS Code（推奨）
...
```powershell
setx ENV "dev"
setx DATABASE_URL "sqlite:///./[ProjectName].db"
...
- request/response body をログしない（●●ゼロ）
- 主要エンドポイントの●●テスト（status/error_code）
...
- Health Check エンドポイントを確認：
  ```powershell
  curl https://[ProjectName]-backend.onrender.com/api/v1/health
  # Expected: {"status":"ok"}
  ```
- Version エンドポイントを確認：
  ```powershell
  curl https://[ProjectName]-backend.onrender.com/api/v1/version
...
- `APP_ENV=production`
- `AI_PROVIDER=openai`
- `DATABASE_URL`（PostgreSQL接続文字列、自動●●）
- `REDIS_URL`（Redis接続文字列、自動●●）
- レート制限設定（`RL_*`）
- 日次制限（`●●_GENERATE_DAILY_LIMIT=3`, `PRO_GENERATE_DAILY_LIMIT=100`）

### 5.6 ログ運用（●●ゼロ）
- Render Logs に●●が出力されていないことを確認
- 例外ログにも●●が混入していないことを確認
- 必要に応じて `UVICORN_ACCESS_LOG=false` で詳細ログを●●（既定値）

### 5.7 コスト試算（Render Starter Plan）
- Web Service: $7/月（512MB RAM、常時●●）
...
- 開発/検証の「ライブ●●」は手動で行う（自動化しない）
- 回数上限は別Spec（●●上限）に従う
- ●●テストに使う●●は、ユーザー同意のあるテストデータのみ（●●非保存）
...
- venvが有効化されているか確認
- `pip install -e .` / `pip install -r requirements●●` を再実行
...
- Render Logs で詳細を確認
- `requirements●●` の依存関係エラー → ローカルで `pip install -r backend/requirements●●` を実行して検証
...
- Render Dashboard → `[ProjectName]-db` → Status を確認
...
- **Blueprint 定義**: ●●ジェクトルートの `render.yaml` にインフラ構成を定義済み
