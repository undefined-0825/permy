...
const List<DiagnosisQuestion> diagnosisQuestions = [
  DiagnosisQuestion(
    ...
    choices: [
      ...
      DiagnosisChoice('social_trust', '評価・信頼'),
      DiagnosisChoice('self_autonomy', '自分の裁量・自由'),
    ],
  ),
  DiagnosisQuestion(
    ...
    title: '迷った時の判断軸は？',
    choices: [
      DiagnosisChoice('low_stress', '無理が少ない方'),
      DiagnosisChoice('long_term_return', '長期的に得な方'),
      DiagnosisChoice('emotional_satisfaction', '気持ちが満たされる方'),
      DiagnosisChoice('pace_control', '自分のペースを守れる方'),
    ],
  ),
  DiagnosisQuestion(
    id: 'night_goal_primary',
    title: '今日の返信で一番達成したいことは？',
    choices: [
      DiagnosisChoice('next_visit', '次回来店の約束'),
      DiagnosisChoice('relationship_keep', '関係維持'),
      DiagnosisChoice('special_distance', '特別感で距離を縮める'),
      DiagnosisChoice('long_term_growth', '長期で太く育てる'),
    ],
  ),
  DiagnosisQuestion(
    ...
    title: '返信の温度感は？',
    choices: [
      DiagnosisChoice('calm_safe', '安心感重視'),
      DiagnosisChoice('sweet_light', '軽く甘め'),
      DiagnosisChoice('clear_proposal', '提案をはっきり'),
  ...
  DiagnosisQuestion(
    ...
    choices: [
      ...
      DiagnosisChoice('care_existing', '今ある関係を丁寧に維持'),
      DiagnosisChoice('focus_key_clients', '重点客に寄せる'),
  ...
  DiagnosisQuestion(
    ...
    title: 'リスク場面での返し方は？',
    choices: [
      DiagnosisChoice('firefighting_safe', 'まず火消しして安全着地'),
      DiagnosisChoice('soft_distance', '柔らかく距離を保つ'),
      DiagnosisChoice('recover_initiative', '主導権を戻す'),
      DiagnosisChoice('adaptive_landing', '相手に合わせて着地'),
    ],
  ),
];
